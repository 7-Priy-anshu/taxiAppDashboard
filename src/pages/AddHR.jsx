...
export default function AddHR() {
      ...
      try {
        ...
        if (id) {
          ...
          setInitialValues({
            name: hrData.name || "",
            email: hrData.email || "",
            password:hrData.password ||"", // Avoid sending password unless updated
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      if (id) {
        await axios.put(`${VITE_API}update/client/person/${id}`, payload, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
      ...
      <div className="w-full max-w-4xl bg-white p-6 sm:p-10 rounded-xl border-t border-t-gray-300 shadow-md">
        ...
        {submitError && (
          <div className="text-red-500 bg-red-100 p-3 rounded mb-4 flex justify-between items-center">
            {submitError}
            <button onClick={() => setSubmitError(null)} className="text-lg">
              ✕
            </button>
          </div>
        )}
          ...
          {({ isSubmitting, values, setFieldValue }) => (
            <Form className="grid grid-cols-1 sm:grid-cols-2 gap-4" action="/formData" method="POST">
                  ...
                  {groupPermissions.map((group) => (
                    <div key={group.group} className="rounded-lg p-4 shadow-sm">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        >
                          {group.items.every((item) => values.permissions.includes(item._id))
                            ? "Unselect All"
                            : "Select All"}
                        </button>
                      </div>
              ...
              <div className="col-span-full flex justify-end gap-4 mt-6">
                <button
                  type="button"
                  onClick={() => navigate("/main-admin/viewAdmin")}
...
//     try {
//       if (id) {
//         await axios.put(`${VITE_API}update/client/clientId/person/${id}`, values,{
//           headers:{
//             Authorization: `Bearer ${token}`,
//           }
//         });
//       } else {
//         await axios.post(`${VITE_API}add/client/clientId/person`, values,{
//           headers:{
//                     Authorization: `Bearer ${token}`,
//           }
//         });
