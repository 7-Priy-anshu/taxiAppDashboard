  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      if (values.email === "superadmin@gmail.com") {
        ...
        res = await axios.post(`${VITE_API}person/login`, values, {
          withCredentials: true,
        });
      ...
      console.log("Login response:", { userData, token }); // Debug full response
      if (!userData || !userData.role || !token) {
        throw new Error("Invalid login response: Missing user or token");
      }

      const fullUserData = { ...userData, email: values.email };
      console.log("Logged in user:", fullUserData); // Log full user data
...
//       // Add email to user data if needed
//       const fullUserData = { ...userData, email: values.email };
// // In handleSubmit, after login:
// console.log("Logged in user PersonChecking:", fullUserData);
// login(fullUserData, token);
//       // Save via AuthContext (handles localStorage too)
//       login(fullUserData, token);
